  ...
  new GitHubStrategy(
    {
      ...
      callbackURL: 'http://107.170.195.210/auth/github/callback'
    },
    function(accessToken, refreshToken, profile, done) {
      userController.findOrCreateGitHubUser(
        profile,
        accessToken,
        refreshToken,
        function(err, user) {
    ...
    userController.findUser(username, function(err, user) {
      ...
      if (!user) {
        ...
        if (user.githubtoken !== 'false') {
            ...
            message: 'Please sign in with your GitHub account.'
          });
        } else {
          userController.validatePassword(user.password, password, function(
            matched
          ) {
            if (matched) {
              done(null, user);
