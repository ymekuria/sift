...
describe('Authenticating Users', function() {
  ...
  beforeEach(function(done) {
    ...
    client.query(
      'CREATE TABLE IF NOT EXISTS users (' +
        ...
        'github VARCHAR(120) DEFAULT false )',
      function(err, result) {
        if (err) {
          throw new Error(err);
        }
  ...
  it('Should write to the database', function(done) {
    ...
    client.query(
      ...
      [user.email, user.displayName, user.password, user.email, user.salt],
  ...
  it('Should return 403 if a username is already in the database', function(
    done
  ) {
    request(
      {
    ...
    request(
      ...
      function(error, res, body) {
        ...
        client.query(queryString, queryArgs, function(err, results) {
          ...
          bcrypt.compare('p@ssw0rd', results.rows[0].password, function(
            err,
            isMatch
          ) {
