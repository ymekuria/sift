...
describe('Posting User Schemas', function() {
  ...
  beforeEach(function(done) {
    ...
    client.query(
      'CREATE TABLE IF NOT EXISTS yonim_test (' +
    ...
    client.query(
      'INSERT INTO yonim_test (firstname, lastname, streetaddress) VALUES ($1, $2, $3)',
  ...
  it('Should retrieve a list of all of a Users schemas', function(done) {
    client.query(
      'INSERT INTO userstables (username, tablename) VALUES ($1, $2)',
      ['yonim', 'yonim_test'],
      function(err, results) {
        if (err) {
          throw new Error(err);
  ...
  it('Should update a value in a usersTable', function(done) {
    request(
      ...
      function(err, results, body) {
        ...
        expect(results.body).to.equal(
          'succesfully modified Yoni to john in yonim_test'
        );
        done();
      }
    );
